...
export async function submitAnswer(token: string, selectedOptionIndex: number) {
  ...
  const { questionId, nonce, seriesSlug } = payload;
  ...
  let zenkaiBoosts = user.zenkai_boosts;
  let activeZenkaiMultiplier = user.active_zenkai_multiplier;
  let zenkaiAttemptsLeft = user.zenkai_attempts_left;
  let consecutiveWrong = 0; // We'd ideally track this in user, but let's assume it's part of leaderboard state or simplified user state

  // Because user record might lack consecutive_wrong, let's keep it simple:
  // If correct, streak increases, multiplier used and reset.
  // If wrong, streak breaks.
  ...
  if (isCorrect) {
    ...
    activeZenkaiMultiplier = 1; // Unlucky
  }
  ...
  try {
    ...
    // Attempt updating leaderboard (using Admin to avoid client restrictions)
    try {
      const records = await adminClient.collection('leaderboard').getList<LeaderboardRecord>(1, 1, {
        filter: `user = "${user.id}"`,
      });
      if (records.items.length > 0) {
        const lbRecord = records.items[0];
        ...
        await adminClient.collection('leaderboard').create({
          ...
          score: newPowerLevel,
          streak: newStreak,
          consecutive_wrong: isCorrect ? 0 : 1
        });
      }
    } catch(err) {
